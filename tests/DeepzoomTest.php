...
class DeepzoomTest extends TestCase
{
    ...
    public function testExifOrientationSwapsDimensions(
        ...
        $dz = new Deepzoom($config);
        $result = $dz->process($this->fixtureExif6ImagePath(), $dest);
        $this->assertTrue($result);

        $dziFiles = glob($dest . '/*.dzi');
        $this->assertNotEmpty($dziFiles, "No .dzi by $processor");
        ...
        $this->assertSame(60, $dzi['width'],
            "Width should be 60 after auto-orient ($processor)");
        $this->assertSame(100, $dzi['height'],
    ...
    public function testNoRotateKeepsRawDimensions(
        ...
        $this->assertSame(100, $dzi['width'],
            "Width should be 100 with noRotate ($processor)");
        $this->assertSame(60, $dzi['height'],
